...
public class Settings: Codable {
    ...
    public required init(from decoder: Decoder) throws {
        ...
        case 1:
            ...
            paliSpeech = try container.decodeIfPresent(SpeechConfig.self, forKey: .paliSpeech) ?? SpeechConfig(language: .default)
            docSpeech = try container.decodeIfPresent(SpeechConfig.self, forKey: .docSpeech) ?? SpeechConfig(language: .default)
            isDarkModeEnabled = try container.decodeIfPresent(Bool.self, forKey: .isDarkModeEnabled) ?? false
            lastApplicationVersion = try container.decodeIfPresent(String.self, forKey: .lastApplicationVersion) ?? ""
            maxDoc = try container.decodeIfPresent(Int.self, forKey: .maxDoc) ?? MAX_DOC_DEFAULT
        default:
