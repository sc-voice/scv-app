...
public class SuttaCentralId: CustomStringConvertible {
  ...
  public init(_ scid: String?) throws {
    guard let scid = scid else {
      throw SuttaCentralIdError.invalidId("required scid:\(String(describing: scid))")
    }
    self.scid = scid
  }
  ...
  /// Matches a SCID against a pattern (supports ranges and comma-separated patterns)
  ...
  static func rangeHigh(_ scid: String) -> String {
    ...
    let suffix = slashParts.count > 1 ? slashParts.dropFirst().joined(separator: "/") : ""
    ...
    let c1Parts = extRanges.count > 1 ? extRanges[1].components(separatedBy: ":") : []
  ...
  static func test(_ text: String) -> Bool {
    ...
    return commaParts.reduce(true) { acc, part in
      let normalized = part.replacingOccurrences(of: #"\. *"#, with: ".", options: .regularExpression)
  ...
  static func languages(_ text: String) -> [String] {
    guard Self.test(text) else { return [] }
  ...
  /// Parses a part identifier (handles ranges like "1-5" and letter suffixes like "1a")
  static func partNumber(_ part: String, _ id: String) throws -> [Int] {
    ...
    if caretParts.count == 1 {
      ...
      guard let n0 = Int(c0dig) else {
        throw SuttaCentralIdError.parseError("partNumber() cannot parse \(part) in \(id)")
      }
  ...
  static func scidNumbersLow(_ idOrPath: String) throws -> [Int] {
    ...
    let nums = try dotParts.reduce([Int]()) { acc, n in
      ...
      return acc + (try Self.partNumber(lowPart, idOrPath))
    }
  ...
  static func scidNumbersHigh(_ idOrPath: String) throws -> [Int] {
    ...
    let nums = try dotParts.reduce([Int]()) { acc, n in
      ...
      return acc + (try Self.partNumber(highPart, idOrPath))
    }
  ...
  public var groups: [String]? {
    guard let tokens = scid.components(separatedBy: ":") as [String]?, tokens.count > 1 else {
      return nil
    }
