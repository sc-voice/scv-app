...
struct SuttaPlayerTests {
  ...
  @Test
  @MainActor
  func prepareSuttaAudioQueuesAllSegments() async throws {
    let mockSynthesizer = MockSpeechSynthesizerForSuttaPlayer()
    let player = SuttaPlayer(synthesizer: mockSynthesizer)
  ...
  @Test
  @MainActor
  func prepareSuttaAudioDoesNotTriggerPlayback() async throws {
    let mockSynthesizer = MockSpeechSynthesizerForSuttaPlayer()
    let player = SuttaPlayer(synthesizer: mockSynthesizer)
    ...
    // Verify queueSynthesisOnly was called, NOT playText
    #expect(mockSynthesizer.queueSynthesisOnlyWasCalled == true)
  ...
  @Test
  @MainActor
  func prepareSuttaAudioProgressCallback() async throws {
    let mockSynthesizer = MockSpeechSynthesizerForSuttaPlayer()
    let player = SuttaPlayer(synthesizer: mockSynthesizer)
    ...
    // Last report should show all segments processed
    let lastReport = progressReports.last
    #expect(lastReport?.0 == lastReport?.1, "Final progress should be at 100%")
    cc.ok1(#line, "passed - \(progressReports.count) progress updates")
  }

  @Test
  @MainActor
  func prepareSuttaAudioErrorsWhenDocumentNotFound() async {
    let mockSynthesizer = MockSpeechSynthesizerForSuttaPlayer()
    let player = SuttaPlayer(synthesizer: mockSynthesizer)
    ...
    do {
      ...
      if case .documentNotFound = error {
        cc.ok1(#line, "passed - documentNotFound thrown as expected")
      } else {
        #expect(Bool(false), "Expected documentNotFound, got different error")
      }
    } catch {
      #expect(Bool(false), "Expected SuttaPlayerError")
    }
  }
}
