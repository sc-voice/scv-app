...
struct Tips {
    ...
    /// - Parameter cardManager: CardManager instance (uses CardManager.shared if nil)
    @MainActor
    static func active(_ action: TipsAction, cardManager: (any ICardManager)? = nil) -> Bool {
...
/// Animates a view with a wiggle and color transition when Tips.active() is true
...
extension View {
    /// Applies Tips.clickMe animation when Tips.active() is true for the given action
    /// - Parameter action: The tip action to check
    /// - Parameter cardManager: CardManager instance (uses CardManager.shared if nil)
    /// - Wiggle: Up/down motion over 1 second for emphasis
    /// - Color: 3-second transition to theme accent color
    func clickMe(_ action: TipsAction, cardManager: (any ICardManager)? = nil) -> some View {
